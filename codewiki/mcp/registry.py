...
_register(
    Tool(
        ...
        description=(
            ...
            "drafts in the draft zone repowiki/skills/ (skill-creator T2+T3, "
            "issues #25/#26, ADR-0004 two-zone gate: drafts are indexed and "
            "linted but NEVER effective — installation to .codebuddy/skills/ "
            "is a separate user action). Mode C protocol — the host agent "
            "writes, the tool does deterministic bookkeeping. mode='prepare' "
            "(zero side effects): candidate materials not yet absorbed by any "
            "skill (scenario blocks + stable pitfall/lesson/decision notes), "
            "open issues grouped per existing skill, the draft-zone skill "
            "index, a name/description conflict pre-check (token Jaccard > "
            "0.6), a graded capacity warning (green / orange=update-only / "
            ...
            "report (no_action) is a legal round. mode='install' name=<slug>: "
            "strip ALL management frontmatter and write {name, description} + "
            "body into the effect zone .codebuddy/skills/<name>/ (IDE-"
            "discovered); stamps installed_at/installed_to/installed_hash on "
            "the draft — the normalized hash (name+description+body) is the "
            "drift-detection contract; idempotent. mode='retire' "
            "name=<slug> reason=<why>: mark the draft deprecated (body kept "
            "for audit, revision records the reason), remove the effect-zone "
            "copy; deprecated drafts refuse re-install. Task memories are NOT "
