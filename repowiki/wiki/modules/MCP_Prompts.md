...
| `_prompt_task_workflow` | 函数 | codewiki/mcp/prompts.py | 任务记忆全流程（关联任务→采集→蒸馏→确认）指引，补蒸馏委托 subagent、**阻塞式先记忆后回答** |
