...
> **Operating Thesis**: 工具做确定性簿记，推理决策在调用方与用户手里；入库必经显式确认闸门。

## Core Principles
- 无状态工具 + LLM 外置：提取/聚合/压缩走 prepare→推理→submit，工具不持模型。
- 确认闸门对等：落盘知识先 draft、confirm 生效，绝不静默确认。
- 显式优于缓存：路径与归属「显式参数 > 布局推导 > session 缓存」。
- 单点收敛：同构逻辑只留一份，新逻辑先找收敛点。
- 候选必有去向：一律落 absorbed/deferred/excluded，排除必填原因。

## Reusable SOPs
- 知识聚合：先问用户 → prepare 取清单与预警 → 分组写场景块（UPDATE>MERGE>CREATE）→ 退役吸收笔记 → submit → lint。
- 竞品调研：克隆源码读代码而非文档站，结论带文件:行号，借鉴先过代码核对，明确不借什么。
- 多文件批处理：逐文件处理 → 落盘 → 立即压缩上下文。
- 新增 MCP 工具：handler + registry schema 两处，交付前跑全量测试。
- 归因调优：先实测计数再下结论，直觉根因须被数据证伪。

## Decision Logic
- 收敛通用层前先枚举调用点确认语义一致，否则只改收口点。
- 成本可见性优先于新造能力：先让调用方看到代价再按需展开。
- 语义不同的信号并存不去重；合并判定 related ≠ same。
- 锚点选可观测稳定 ID 与既有元数据；捷径分支也须持久化。
- 文档站是宣传口径，代码是事实口径，冲突时代码优先；竞品机制价值取决于自身架构前提。

## Boundaries
- 不自动蒸馏/聚合/刷新 Doctrine：hint 先问用户，触发永远显式。
- 不把中文/结构化文本交给 shell 中间层：一律落文件或用精确 subprocess。
- 不依赖下游幂等当安全网。
- 不绕过 dispatch/schema 校验直连 handler；改契约先分清校验层。
- 不静默失败：配置损坏打日志；未实现的兜底要么做要么删。
- 不假设数据形状，先读格式再动手；近似输出不当精确结论。
- 依赖升级勿顺手放宽 lint。

## Agent Rules
- 调用工具前先读描述确认参数名；路径显式传 repo_path。
- 子代理自报结果须独立复核后再转述。
- 重提交必须携带完整正文，只带裁决说明会覆盖原文。
- 重型分析先收窄范围；聚合后必跑 lint。
- 注入/采集代码 fail-open：永不非零退出、缺依赖降级。

---

> 更新：2026-09-18 · 场景：10 · 笔记：74
